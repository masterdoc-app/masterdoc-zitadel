...
Workflow **Terraform Apply** (`workflow_dispatch`) — project `masterdoc-toir`, feature keys (`board`, `charts`, `equipment`, `admin`, `black_box`), OIDC apps (`masterdoc-kmp-native`, `masterdoc-kmp-web`), login policy (no self-signup).
